...
def make_type_array(data, file_data, type_name):
    ...
    if type_name == 'normal':
        ...
        for i in range(len(data)):            
            if data['subID'][i] in file_data:
                ...
                this_tmp_labels = tmp['Label']
                one_count = list(this_tmp_labels).count('1')
                
                tmp2 = data[data['subID'] == data['subID'][i]]
                this_tmp2_labels = tmp2['Label']
                zero_count = list(this_tmp2_labels).count('0')
                
                if one_count > 0:
                    if data['subID'][i] not in id_data:
                        id_data.append(data['subID'][i])
                        tt = np.array(tmp['Label'])
                        label_data.append(tt)
                
                elif zero_count == 6:
                    if data['subID'][i] not in id2_data:
                        if len(id2_data) < len(id_data)/10:
                            id2_data.append(data['subID'][i])
                            tt2 = np.array(tmp2['Label'])
                            label2_data.append(tt2)
    ...
    id2_data = np.array(id2_data, dtype = str)
    label2_data = np.array(label2_data)
    
    #id_array = id_data[0:len(id_data) - 2]
    #label_array = id_data[0:len(label_data) - 2]
    
    id_array = np.hstack([id_data, id2_data])
    label_array = np.vstack([label_data, label2_data])
